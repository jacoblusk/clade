...
ULONG
CRC32(LPWSTR lpszKey, SIZE_T cLength) {
    UINT i;
    ULONG crc32Value = 0;
  
    /* Should we use WCHAR_MAX? */
    for(i = 0; i < cLength; i++) {
        crc32Value = CRC32TABLE[(crc32Value ^ lpszKey[i]) & 0xFF] ^ (crc32Value >> 8);
    }
...
HashMap_Get(PHASHMAP pHashMap, LPWSTR lpszKey, LPVOID *ppValue) {
	...
	for(i = 0; i < MAX_CHAIN_LENGTH; i++) {
        ...
        if(bInUse) {
            if(lstrcmpiW(pHashMap->m_aElements[curr].m_lpszKey, lpszKey) == 0) {
	...
	for(i = 0; i < MAX_CHAIN_LENGTH; i++){
        ...
        if (bInUse){
            if(lstrcmpiW(pHashMap->m_aElements[curr].m_lpszKey, lpszKey) == 0) {
                ...
                pHashMap->m_aElements[curr].m_bInUse = 0;
                pHashMap->m_aElements[curr].m_pData = NULL;
                pHashMap->m_aElements[curr].m_lpszKey = NULL;
